 ...
 *
 * Disassembly of DSDT.aml, Tue Apr  3 20:53:09 2018
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0001027B (66171)
 *     Revision         0x02
 *     Checksum         0x13
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "CB-01   "
            ...
            Device (BAT0)
            {
                ...
                Method (_BIF, 0, NotSerialized)  // _BIF: Battery Information
                {
                    If (LEqual (ECON, One))
                    {
                        ...
                        Store (B1B2 (BDV0, BDV1), Index (PBIF, 0x04))
                ...
                Method (_BST, 0, Serialized)  // _BST: Battery Status
                {
                    ...
                    If (LEqual (ECON, One))
                    {
                        ...
                        Store (OBAC, OBPR)
                        Sleep (0x10)
                        Store (B1B2 (BRC0, BRC1), OBRC)
                        Sleep (0x10)
                        Store (B1B2 (BFV0, BFV1), OBPV)
            ...
            Scope (\_SB.PCI0.LPCB.EC0)
            {
                Device (VPC0)
                {
                    ...
                    Method (SMTF, 1, NotSerialized)
                    {
                        If (LEqual (Arg0, Zero))
                        {
                            If (LEqual (B1B2 (BFV0, BFV1), Zero))
                            {
                                Return (Zero)
                            }

                            If (LEqual (B1B2 (BAC0, BAC1), Zero))
                            {
                                Return (Zero)
                            }
                            ...
                            {
                                Store (B1B2 (BFV0, BFV1), VBPV)
                                Store (B1B2 (BAC0, BAC1), VBAC)
        ...
        Method (_OSC, 4, Serialized)  // _OSC: Operating System Capabilities
        {
            ...
            If (LEqual (Arg0, ToUUID ("0811b06e-4a27-44f9-8d60-3cbbc22e7b48") /* Platform-wide Capabilities */))
            {
                If (LEqual (Arg1, One))
                {
                    ...
                    If (LOr (XOSI ("Windows 2013"), XOSI ("Windows 2015")))
                    {
                        Or (CAP0, 0x04, CAP0)
                        Store (STS0, OSCI)
                        Store (CAP0, OSCO)
